...
进一步可以证明，协方差矩阵是`半正定矩阵`，下面对连续型概率分布进行证明，对于离散型概率分布方法类似

假设有$n$维随机向量$x$，其联合概率密度函数为$f(\boldsymbol{x})$，协方差矩阵为$\boldsymbol{\Sigma}$，对于任意非$\boldsymbol{0}$向量$\boldsymbol{y}$，有
$$
\begin{aligned}
y^{\mathrm{T}} \Sigma y & =\sum_{i=1}^{n} \sum_{j=1}^{n} y_{i} y_{j} \operatorname{cov}\left(x_{i}, x_{j}\right)=\sum_{i=1}^{n} \sum_{j=1}^{n} y_{i} y_{j} \int_{\mathbb{R}^{n}}\left(x_{i}-\mu_{i}\right)\left(x_{j}-\mu_{j}\right) f(\boldsymbol{x}) \mathrm{d} \boldsymbol{x} \\
& =\int_{\mathbb{R}^{n}}\left(\sum_{i=1}^{n} \sum_{j=1}^{n} y_{i} y_{j}\left(x_{i}-\mu_{i}\right)\left(x_{j}-\mu_{j}\right)\right) f(\boldsymbol{x}) \mathrm{d} \boldsymbol{x}=\int_{\mathbb{R}^{n}}\left(\sum_{k=1}^{n} y_{k}\left(x_{k}-\mu_{k}\right)\right)^{2} f(\boldsymbol{x}) \mathrm{d} \boldsymbol{x} \geqslant 0
\end{aligned}
$$
...
$$
p(\boldsymbol{x})=\frac{1}{(2 \pi)^{\frac{n}{2}}|\boldsymbol{\Sigma}|^{\frac{1}{2}}} \exp \left(-\frac{1}{2}(\boldsymbol{x}-\boldsymbol{\mu})^{\mathrm{T}} \boldsymbol{\Sigma}^{-1}(\boldsymbol{x}-\boldsymbol{\mu})\right)
$$
...
联合概率密度函数为
$$
p(\boldsymbol{x})=\frac{1}{(2 \pi)^{\frac{\pi}{2}}} \exp \left(-\frac{1}{2} \boldsymbol{x}^{\mathrm{T}} \boldsymbol{x}\right)
$$
...
$$
p(x, y)=\frac{1}{2 \pi \sigma_{1} \sigma_{2} \sqrt{1-\rho^{2}}} \exp \left(-\frac{1}{2\left(1-\rho^{2}\right)}\left(\frac{\left(x-\mu_{1}\right)^{2}}{\sigma_{1}^{2}}-\frac{2 \rho\left(x-\mu_{1}\right)\left(y-\mu_{2}\right)}{\sigma_{1} \sigma_{2}}+\frac{\left(y-\mu_{2}\right)^{2}}{\sigma_{2}^{2}}\right)\right)
$$
其均值向量为
$$
...
算法需要根据一组服从此概率分布的样本来估计出概率分布的参数，称为`参数估计`问题
...
最大似然估计认为使得观测数据(样本集)出现概率最大的参数为最优参数，这一方法体现了**存在的就是合理的**这一朴素的哲学思想：既然这组样本出现了，那么它们出现的概率理应是最大化的
...
$$
\prod_{i=1}^{l} p\left(\boldsymbol{x}_{i} ; \boldsymbol{\theta}\right)
$$
这个联合概率也称为似然函数，其中$x_{i}$是已知量，$\theta$是待确定的末知数，似然函数是优化变量$\theta$的函数
$$
L(\boldsymbol{\theta})=\prod_{i=1}^{l} p\left(\boldsymbol{x}_{i} ; \boldsymbol{\theta}\right)
$$
目标是让该函数的值最大化，这样做的依据是这组样本出现了，因此应该最大化它们出现的概率

即求解如下最优化问题
$$
\max _{\boldsymbol{\theta}} \prod_{i=1}^{l} p\left(\boldsymbol{x}_{i} ; \boldsymbol{\theta}\right)
$$
求解驻点方程可以得到问题的解

乘积求导不易处理且连乘容易造成浮点数溢出，将似然函数取对数，得到对数似然函数
$$
\ln L(\boldsymbol{\theta})=\ln \prod_{i=1}^{l} p\left(\boldsymbol{x}_{i} ; \boldsymbol{\theta}\right)=\sum_{i=1}^{l} \ln p\left(\boldsymbol{x}_{i} ; \boldsymbol{\theta}\right)
$$
对数函数是增函数，因此最大化似然函数等价于最大化对数似然函数

最后要求解的问题为
$$
\max _{\boldsymbol{\theta}} \sum_{i=1}^{L} \ln p\left(\boldsymbol{x}_{i} ; \boldsymbol{\theta}\right)
$$
...
![伯努利分布的似然函数](https://pic.hycbook.com/i/hexo/bk_resources/math/机器学习_概率论/伯努利分布的似然函数.svg)
...
对于正态分布$N\left(\mu, \sigma^{2}\right)$，有样本集$$x_{1}, \cdots, x_{n}$$，该样本集的似然函数为
$$
L(\mu, \sigma)=\prod_{i=1}^{n} \frac{1}{\sqrt{2 \pi} \sigma} \exp \left(-\frac{\left(x_{i}-\mu\right)^{2}}{2 \sigma^{2}}\right)=\left(2 \pi \sigma^{2}\right)^{-\frac{n}{2}} \exp \left(-\frac{1}{2 \sigma^{2}} \sum_{i=1}^{n}\left(x_{i}-\mu\right)^{2}\right)
$$
对数似然函数为
$$
\ln L(\mu, \sigma)=-\frac{n}{2} \ln (2 \pi)-\frac{n}{2} \ln \left(\sigma^{2}\right)-\frac{1}{2 \sigma^{2}} \sum_{i=1}^{n}\left(x_{i}-\mu\right)^{2}
$$
对$\mu$和$\sigma$求偏导数并令其为0，得到下面的方程组
$$
\left\{\begin{array}{l}
\frac{\partial \ln L(\mu, \sigma)}{\partial \mu}=\frac{1}{\sigma^{2}} \sum_{i=1}^{n}\left(x_{i}-\mu\right)=0 \\
\frac{\partial \ln L(\mu, \sigma)}{\partial \sigma}=-\frac{n}{\sigma}+\frac{1}{\sigma^{3}} \sum_{i=1}^{n}\left(x_{i}-\mu\right)^{2}=0
\end{array}\right.
$$
解得
$$
\mu=\bar{x}=\frac{1}{n} \sum_{i=1}^{n} x_{i} \qquad \sigma^{2}=\frac{1}{n} \sum_{i=1}^{n}\left(x_{i}-\mu\right)^{2}
$$
...
有$n$维正态分布$N(\boldsymbol{\mu}, \boldsymbol{\Sigma})$，给定一组样本$$x_{1}, \cdots, \boldsymbol{x}_{l}$$，其对数似然函数为
$$
\begin{aligned}
\ln L(\boldsymbol{\mu}, \boldsymbol{\Sigma}) & =\ln \prod_{i=1}^{l} \frac{1}{(2 \pi)^{\frac{n}{2}}|\boldsymbol{\Sigma}|^{\frac{1}{2}}} \exp \left(-\frac{1}{2}\left(\boldsymbol{x}_{i}-\boldsymbol{\mu}\right)^{\mathrm{T}} \boldsymbol{\Sigma}^{-1}\left(\boldsymbol{x}_{i}-\boldsymbol{\mu}\right)\right) \\
& =-\frac{n l}{2} \ln (2 \pi)-\frac{l}{2} \ln |\boldsymbol{\Sigma}|-\frac{1}{2} \sum_{i=1}^{1}\left(\boldsymbol{x}_{i}-\boldsymbol{\mu}\right)^{\mathrm{T}} \boldsymbol{\Sigma}^{-1}\left(\boldsymbol{x}_{i}-\boldsymbol{\mu}\right)
\end{aligned}
$$
对$\boldsymbol{\mu}$求梯度并令梯度为0
$$
\nabla_{\mu} \ln L=\sum_{i=1}^{l} \Sigma^{-1}\left(x_{i}-\mu\right)=\mathbf{0}
$$
两边左乘$\boldsymbol{\Sigma}$，解得
$$
\mu=\frac{1}{l} \sum_{i=1}^{l} x_{i}
$$
对$\Sigma$的求解更为复杂，因火它要满足`对称正定性约束`条件，可以解得
$$
\Sigma=\frac{1}{l} \sum_{i=1}^{1}\left(x_{i}-\mu\right)\left(x_{i}-\mu\right)^{\mathrm{T}}
$$
...
$$
p(\boldsymbol{\theta} \mid \boldsymbol{x})=\frac{p(\boldsymbol{x} \mid \boldsymbol{\theta}) p(\boldsymbol{\theta})}{p(\boldsymbol{x})}=\frac{p(\boldsymbol{x} \mid \boldsymbol{\theta}) p(\boldsymbol{\theta})}{\int_{\boldsymbol{\theta}} p(\boldsymbol{x} \mid \boldsymbol{\theta}) p(\boldsymbol{\theta}) \mathrm{d} \boldsymbol{\theta}}
$$
其中$p(x \mid \theta)$是给定参数值时样本的概率分布，就是$x$的概率密度函数或概率质量函数，可以根据样本的值$x$进行计算，与最大似然估计相同，$\theta$是随机变量，因此，最大化该后验概率等价于
$$
\arg \max _{\boldsymbol{\theta}} p(\boldsymbol{\theta} \mid \boldsymbol{x})=\arg \max _{\boldsymbol{\theta}} \frac{p(\boldsymbol{x} \mid \boldsymbol{\theta}) p(\boldsymbol{\theta})}{\int_{\boldsymbol{\theta}} p(\boldsymbol{x} \mid \boldsymbol{\theta}) p(\boldsymbol{\theta}) \mathrm{d} \boldsymbol{\theta}}=\arg \max _{\boldsymbol{\theta}} p(\boldsymbol{x} \mid \boldsymbol{\theta}) p(\boldsymbol{\theta})
$$
...
![伯努利分布参数估计目标函数](https://pic.hycbook.com/i/hexo/bk_resources/math/机器学习_概率论/伯努利分布参数估计目标函数.svg)
...
$$
\frac{3}{p}-\frac{7}{1-p}-100(p-0.3)=0
$$
由于$0<p<1$，因此解此方程即可得到$p$的值

最大值在$0.3$点附近取得，与上图有所差异，图中横坐标为参数$p$，纵坐标为目标函数值

接下来计算正态分布的参数，假设有正态分布$$N\left(\mu_{1} \sigma_{v}^{2}\right)$$，其均值$$\mu$$末知，而方差已知

有组采样自该分布的独立同分布样本$$x_{1}, \cdots, x_{n}$$，假设参数$$\mu$$服从从正态分布$$N\left(\mu_{0}, \sigma_{m}^{2}\right)$$

最大后验概率估计的目标函数为
$$
L(\mu)=\frac{1}{\sqrt{2 \pi} \sigma_{m}} \exp \left(-\frac{\left(\mu-\mu_{0}\right)^{2}}{2 \sigma_{m}^{2}}\right) \prod_{i=1}^{n} \frac{1}{\sqrt{2 \pi} \sigma_{v}} \exp \left(-\frac{\left(x_{i}-\mu\right)^{2}}{2 \sigma_{v}^{2}}\right)
$$
将该函数取对数, 可得
$$
\ln L(\mu)=\ln \frac{1}{\sqrt{2 \pi} \sigma_{m}}-\frac{\left(\mu-\mu_{0}\right)^{2}}{2 \sigma_{m}^{2}}+n \ln \frac{1}{\sqrt{2 \pi} \sigma_{v}}-\sum_{i=1}^{n} \frac{\left(x_{i}-\mu\right)^{2}}{2 \sigma_{v}^{2}}
$$
最大化此目标函数等价于最小化如下函数
$$
f(\mu)=\frac{\left(\mu-\mu_{0}\right)^{2}}{\sigma_{m}^{2}}+\sum_{i=1}^{n} \frac{\left(x_{i}-\mu\right)^{2}}{\sigma_{v}^{2}}
$$
对$\mu$求导并令导数为0，可以解得
$$
\mu=\frac{\sigma_{m}^{2}\left(\sum_{i=1}^{n} x_{i}\right)+\sigma_{v}^{2} \mu_{0}}{\sigma_{m}^{2} n+\sigma_{v}^{2}}
$$
这就是均值的最大后验概率估计结果
...
$$
p(\boldsymbol{\theta} \mid \boldsymbol{x})=\frac{p(\boldsymbol{x} \mid \boldsymbol{\theta}) p(\boldsymbol{\theta})}{\int_{\boldsymbol{\theta}} p(\boldsymbol{x} \mid \boldsymbol{\theta}) p(\boldsymbol{\theta}) \mathrm{d} \boldsymbol{\theta}}
$$
同样的，$p(\boldsymbol{\theta})$为参数的先验分布，$p(\boldsymbol{x} \mid \boldsymbol{\theta})$为给定参数时样本的概率分布，这里得到的是参数的概率分布，通常取其数学期望作为参数的估计值，即参数的估计值为
$$
E[p(\boldsymbol{\theta} \mid \boldsymbol{x})]
$$
式$p(\boldsymbol{\theta} \mid \boldsymbol{x})$分母中的积分通常难以计算，`信息论`章节将讲述参数后验概率的近似计算方法一`变分推断`
...
对于很多应用，我们无法给出概率密度函数的显式表达式，此时可以使用`核密度估计`

核密度估计(Kernel Density Estimation, KDE)也称为`Parzen窗技术`，是一种非参数方法，它无须求解概率密度函数的参数，而是用一组标准函数的叠加表示概率密度函数
...
$$
p(\boldsymbol{x})=\frac{1}{n h^{d}} \sum_{i=1}^{n} K\left(\frac{\boldsymbol{x}-\boldsymbol{x}_{i}}{h}\right)
$$
其中$h$为核函数的窗口半径，是人工设定的正参数

核函数要保证函数值$$K\left(\frac{x-x_{i}}{h}\right)$$随着待估计点$$x$$离样本点$$x_{i}$$的距离增加而递减
...
$$
\int_{\mathbb{R}^{d}} \frac{1}{h^{d}} K\left(\frac{\boldsymbol{x}-\boldsymbol{x}_{i}}{h}\right) \mathrm{d} \boldsymbol{x}=1
$$
核函数能确保积分值为1
$$
\int_{\mathbb{R}^{d}} K(\boldsymbol{y}) \mathrm{d} \boldsymbol{y}=1
$$
如果令$\frac{x-x_{i}}{h}=y$

其逆变换为
$$
\boldsymbol{x}=h \boldsymbol{y}+\boldsymbol{x}_{i}
$$
此换元的雅可比行列式为
$$
\left|\frac{\partial x}{\partial y}\right|=\left|\begin{array}{ccc}
h & \cdots & 0 \\
\vdots & & \vdots \\
0 & \cdots & h
\end{array}\right|=h^{d}
...
因此有
$$
\frac{1}{h^{d}} \int_{\mathbb{R}^{d}} K\left(\frac{\boldsymbol{x}-\boldsymbol{x}_{i}}{h}\right) \mathrm{d} \boldsymbol{x}=\frac{1}{h^{d}} \int_{\mathbb{R}^{d}} K(\boldsymbol{y})\left|\frac{\partial \boldsymbol{x}}{\partial \boldsymbol{y}}\right| \mathrm{d} \boldsymbol{y}=\frac{1}{h^{d}} \int_{\mathbb{R}^{d}} K(\boldsymbol{y}) h^{d} \mathrm{~d} \boldsymbol{y}=1
$$
常用的核函数是径向对称核(Radially Symmetric Kernel)，可以写成如下形式
$$
K(\boldsymbol{x})=c_{k, d} k\left(\|\boldsymbol{x}\|^{2}\right)
$$
其中$k(x)$为核的`剖面(profile)函数`，是$\|\boldsymbol{x}\|$的减函数且对点$x$关于原点径向对称，这也是`径向对称核`这一名称的来历
...
$$
K(\boldsymbol{x})=\left\{\begin{array}{ll}
\frac{1}{2} c_{d}^{-1}(d+2)\left(1-\|\boldsymbol{x}\|^{2}\right) & \|\boldsymbol{x}\| \leqslant 1 \\
0 & \|\boldsymbol{x}\|>1
\end{array}\right.
$$
...
$$
K(\boldsymbol{x})=(2 \pi)^{-d / 2} \exp \left(-\frac{1}{2}\|\boldsymbol{x}\|^{2}\right)
$$
它的归一化系数为$(2 \pi)^{-d / 2}$，因为根据`3.8.3节`的结论
$$
\int_{\mathbb{R}^{d}} \exp \left(-\frac{1}{2}\|\boldsymbol{x}\|^{2}\right) \mathrm{d} \boldsymbol{x}=(2 \pi)^{d / 2}
$$
...
可以写成
$$
f_{h, K}(\boldsymbol{x})=\frac{c_{k, d}}{n h^{d}} \sum_{i=1}^{n} k\left(\left\|\frac{\boldsymbol{x}-\boldsymbol{x}_{i}}{h}\right\|^{2}\right)
$$
对于某些实际应用，需要计算核密度函数的极大值点，典型的是视觉目标跟踪、桃类算法

如果用梯度上升法求解此问题，那么可以得到著名的`均值漂移`(Mean Shift)算法
